...
- **서버가 죽은 채 큐가 '도는 중'이면 폴러가 깨운다(2026-09-15 09:48 재부팅 실사고 — 1시간 무응답).** 종전엔 서버를 새 요청이 올 때만 띄워서, 배치 중 PC가 꺼지면 원장은 `running` 그대로 아무도 안 깨웠다. 이제 포트가 비었고(`orphaned`) 큐에 도는 작업이 있으면 서버를 띄우고, `queue.py` 가 닫은 `서버 재시작으로 중단됨` 요청은 **1회**(`MAX_RESTART_RETRIES`) 사람 재전송 없이 다시 넣는다. 두 번째도 끊기면 실패로 닫는다. 응답만 늦고 포트가 물려 있으면 **깨우지 않는다**(살아 있는 작업을 중단됨으로 덮는다).
